  ...
  const fetcher = useCallback(
    async () =>
      getPerformanceEvaluations({
        relations: ['user_performance_evaluation_user_idTouser', 'user_performance_evaluation_evaluator_idTouser'],
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.evaluation_date ? format(parseISO(record?.evaluation_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'evaluation_score', header: 'evaluation_score', accessorKey: 'evaluation_score' },
    { id: 'comments', header: 'Comments', accessorKey: 'comments' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_performance_evaluation_user_idTouser',
          header: 'User Performance Evaluation User Id Touser',
          accessorKey: 'user_performance_evaluation_user_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_performance_evaluation_user_idTouser?.id}`}
            >
              {record.user_performance_evaluation_user_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_performance_evaluation_evaluator_idTouser',
          header: 'User Performance Evaluation Evaluator Id Touser',
          accessorKey: 'user_performance_evaluation_evaluator_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_performance_evaluation_evaluator_idTouser?.id}`}
            >
              {record.user_performance_evaluation_evaluator_idTouser?.email}
            </Link>
          ),
        }
      : null,
